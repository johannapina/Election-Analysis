...
with open(file_to_save, "w") as txt_file:
    ...
    for counties in county_votes:
        ...
        vote = county_votes.get(counties)
        # 6c: Calculate the percentage of votes for the county.
        county_percentage = float(vote) / float(total_votes) * 100
        county_results = (
            f"{counties}: {county_percentage:.1f}% ({vote:,})\n")
        ...
        if (vote > votes_largestturnout):
            votes_largestturnout = vote
            county_largestturnout = counties
